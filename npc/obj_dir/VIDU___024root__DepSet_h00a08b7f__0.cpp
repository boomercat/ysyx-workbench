// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See VIDU.h for the primary calling header

#include "verilated.h"
#include "verilated_dpi.h"

#include "VIDU__Syms.h"
#include "VIDU___024root.h"

extern "C" void trap_ebreak();

VL_INLINE_OPT void VIDU___024root____Vdpiimwrap_top__DOT__idu__DOT__trap_ebreak_TOP() {
    VL_DEBUG_IF(VL_DBG_MSGF("+    VIDU___024root____Vdpiimwrap_top__DOT__idu__DOT__trap_ebreak_TOP\n"); );
    // Body
    trap_ebreak();
}

#ifdef VL_DEBUG
VL_ATTR_COLD void VIDU___024root___dump_triggers__ico(VIDU___024root* vlSelf);
#endif  // VL_DEBUG

void VIDU___024root___eval_triggers__ico(VIDU___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    VIDU__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    VIDU___024root___eval_triggers__ico\n"); );
    // Body
    vlSelf->__VicoTriggered.at(0U) = (0U == vlSelf->__VicoIterCount);
#ifdef VL_DEBUG
    if (VL_UNLIKELY(vlSymsp->_vm_contextp__->debug())) {
        VIDU___024root___dump_triggers__ico(vlSelf);
    }
#endif
}

#ifdef VL_DEBUG
VL_ATTR_COLD void VIDU___024root___dump_triggers__act(VIDU___024root* vlSelf);
#endif  // VL_DEBUG

void VIDU___024root___eval_triggers__act(VIDU___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    VIDU__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    VIDU___024root___eval_triggers__act\n"); );
    // Body
    vlSelf->__VactTriggered.at(0U) = ((IData)(vlSelf->Reg__02Eclk) 
                                      & (~ (IData)(vlSelf->__Vtrigrprev__TOP__Reg__02Eclk)));
    vlSelf->__VactTriggered.at(1U) = ((IData)(vlSelf->top__02Eclk) 
                                      & (~ (IData)(vlSelf->__Vtrigrprev__TOP__top__02Eclk)));
    vlSelf->__Vtrigrprev__TOP__Reg__02Eclk = vlSelf->Reg__02Eclk;
    vlSelf->__Vtrigrprev__TOP__top__02Eclk = vlSelf->top__02Eclk;
#ifdef VL_DEBUG
    if (VL_UNLIKELY(vlSymsp->_vm_contextp__->debug())) {
        VIDU___024root___dump_triggers__act(vlSelf);
    }
#endif
}
