...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm << 12);//将一个20位立即数加载到目标寄存器的高20位，其余低12位清零。
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 < imm ? 1 : 0));//反编译中seqz = sltiu
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1+src2);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, /*s->dnpc = s->pc+ imm;R(rd) = s->pc +4*/
                                                                s->dnpc = s->pc; s->dnpc += imm; R(rd) = s->pc + 4 );
  INSTPAT("??????? ????? ????? ??? ????? 1110011", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
...
int isa_exec_once(Decode *s) {
  s->isa.inst.val = inst_fetch(&s->snpc, 4);//inst_fetch取值
