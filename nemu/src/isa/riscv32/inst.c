...
//  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));  
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = (sword_t)src1 < (sword_t)imm); //进行有符号数的比较
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, uint32_t a = src1;uint32_t b = src2; uint64_t tmp = (uint64_t)a *(uint64_t)b; R(rd) =BITS(tmp,64,32));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 < imm ? 1 : 0));//反编译中seqz = sltiu
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = CSR(imm);CSR(imm) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = CSR(imm);CSR(imm) |= src1);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, ECALL(s->dnpc));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, printf("this is ebrke");NEMUTRAP(s->pc, R(10))); // R(10) is $a0
